...
describe('getDeviceId — session resolution', () => {
  ...
  it('FORK: skips the live-session lookup for historical events', async () => {
    // A within-window live session exists, but the event is flagged historical
    // (offline/late upload). We must NOT graft the backfill onto the live
    // session — skip the lookup and mint a deterministic bucket id instead, so
    // the worker reconstructs a separate historical session from it.
    const spy = vi
      .spyOn(sessionBuffer, 'getExistingSession')
      .mockResolvedValue(fakeSession('sess-live', NOW - MINUTE));
    ...
    expect(result.deviceId).toBe('cookie-abc');
    expect(result.sessionId).toBeTruthy();
    expect(result.sessionId).not.toBe('sess-live');
  });
});
